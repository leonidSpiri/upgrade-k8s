...
7. Для каждого upgrade-step:
   ...
   - ждёт `Ready`;
   - делает `uncordon`.
8. Если указан `--helm true`, отдельно обновляет Helm через официальный installer script.
...
#### `--kubeconfig`
Путь к kubeconfig, который `kubectl` будет использовать для общения с кластером.
